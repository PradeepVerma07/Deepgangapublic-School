...
class SmStudentCertificateController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
        */
            $certificates = SmStudentCertificate::where('active_status', 1)
    ...
    public function generateCertificateSearch(Request  $request)
    {
        ...
        */
            ...
            $certificates = SmStudentCertificate::where('active_status', 1)->where('school_id',Auth::user()->school_id)->select(['id','name','header_left_text','date','body','body_two','certificate_no','type','footer_left_text','footer_center_text','footer_right_text','student_photo','file','layout','body_font_family','body_font_size','height','width','default_for','active_status'])->get();
            if(moduleStatusCheck('University')){
                ...
                return view('university::un_generate_certificate', compact('graduates', 'certificates', 'certificate_id', 'un_session_id', 'un_faculty_id', 'un_department_id'));
                ...
                $students = StudentRecord::when($request->academic_year, function ($query) use ($request) {
                    ...
                    return view('backEnd.admin.generate_certificate', compact('classes', 'certificates', 'certificate_id', 'certificates', 'students', 'class_id', 'section_id', 'shift_id'));
            }
        /*
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
