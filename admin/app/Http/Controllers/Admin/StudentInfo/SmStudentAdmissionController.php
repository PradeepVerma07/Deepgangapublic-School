...
class SmStudentAdmissionController extends Controller
{
    ...
    public function view(Request $request, $id, $type = null)
    {
        ...
        */
            ...
            $year = $now->year;
            $month = $now->month;
             $days = (int) date('t', strtotime("{$now->month}-{$now->year}-01"));
        ...
        if ($step == 'import') {
            ...
            try {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $dob = $this->parseExcelDate($request->date_of_birth[$i] ?? null);
                    $admission_date = $this->parseExcelDate($request->admission_date[$i] ?? null) ?? date('Y-m-d');

                    if (! $dob) {
                        throw new \Exception('Invalid date of birth format at row '.($i + 1));
                    }
                    ...
                    if (moduleStatusCheck('University')) {
                        $recordData = (object)[
                            ...
                            'un_session_id' => $request->un_session_id ?? null,
                            'un_faculty_id' => $request->un_faculty_id ?? null,
                            'un_department_id' => $request->un_dept_id ?? null,
                            'un_dept_id' => $request->un_dept_id ?? null,
                            'un_academic_id' => $request->un_academic_id ?? null,
                            'un_semester_id' => $request->un_semester_id ?? null,
                            'un_semester_label_id' => $request->un_semester_label_id ?? null,
                            'un_section_id' => $request->un_section_id ?? null,
                            'is_default' => 1,
                            'roll_number' => $request->roll_no[$i] ?? null,
                            'session' => $request->session_id ?? null,
                        ];
                        $this->insertStudentRecord($recordData);
                    } else {
                        $shiftId = null;
                        if (shiftEnable() && !empty($request->shift_id)) {
                            $shiftId = $request->shift_id;
                        }
                        
                        $recordData = StudentRecord::create([
                            'student_id' => $student->id,
                            'class_id' => $request->class_id,
                            'section_id' => $request->section_id,
                            'shift_id' => $shiftId,
                            'academic_id' => $request->session_id,
                            'session_id' => $request->session_id,
                            'is_default' => 1,
                            'roll_no' => $request->roll_no[$i] ?? null,
                            'school_id' => Auth::user()->school_id,
                        ]);
                    }
                }
                ...
                DB::rollBack();
                \Log::error('Student bulk import error at row ' . ($i + 1) . ': ' . $e->getMessage() . '\n' . $e->getTraceAsString());
                Toastr::error('Import failed at row ' . ($i + 1) . ': ' . $e->getMessage(), 'Error');
                return redirect()->back();
            }
        }
    }
