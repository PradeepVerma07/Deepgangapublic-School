        ...
        */
            ...
            if ($request->has('code')) {
                ...
                if ($req_code == 'DevelopedBySpondonit-'.$partision_data[1].'-'.$password) {
                    if (auth()->attempt(['username' => $partision_data[1], 'password' => $password]) || auth()->attempt(['email' => $partision_data[1], 'password' => $password])) {
                        ...
                        if (moduleStatusCheck('University')) {
                            ...
                            if (moduleStatusCheck('University') == false && $user->school->email != SmGeneralSettings::where('school_id', $school)->first()->email) {
                                $session_id = SmGeneralSettings::where('school_id', $school)->first()->session_id;
        ...
        if (count($users) > 0 && count($users) > 1) {
            ...
            foreach ($users as $key => $user) {
                if ($user->school_id) {
                    ...
                    if (Auth::attempt($credentials) && (! Auth::user()->access_status || ! Auth::user()->active_status)) {
                        $this->guard()->logout();
                        Toastr::error('You are not allowed, Please contact with administrator.', 'Failed');
                    ...
                    if (Hash::check($request->password, $user->password)) {
                        ...
                        if (! $request->code) {
                            ...
                            } elseif (mb_strpos($url[1], $user->school->domain) != false) {
