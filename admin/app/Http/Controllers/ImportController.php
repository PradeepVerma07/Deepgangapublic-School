...
class ImportController extends Controller
{
    ...
    public function staffStore(StaffImportRequestForm $staffImportRequestForm)
    {
        try {
            DB::beginTransaction();
            Excel::import(new StaffsImport, $staffImportRequestForm->file('file'), null, \Maatwebsite\Excel\Excel::XLSX);
            ...
            if (! empty($bulk_staffs)) {
                foreach ($bulk_staffs as $bulk_staff) {
                    ...
                    $role_id = InfixRole::where('is_saas', 0)->where('name', $bulk_staff->role)
                        ->where(function ($q): void {
                    ...
                    if ($user) {
                        ...
                        $staff->date_of_birth = $bulk_staff->date_of_birth ?: null;
                        $staff->date_of_joining = $bulk_staff->date_of_joining ?: null;
