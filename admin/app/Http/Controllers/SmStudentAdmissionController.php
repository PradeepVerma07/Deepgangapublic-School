...
class SmStudentAdmissionController extends Controller
{
    ...
    public function admission()
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_admission', ['classes' => $classes, 'religions' => $religions, 'blood_groups' => $blood_groups, 'genders' => $genders, 'route_lists' => $route_lists, 'vehicles' => $vehicles, 'dormitory_lists' => $dormitory_lists, 'categories' => $categories, 'groups' => $groups, 'sessions' => $sessions, 'max_admission_id' => $max_admission_id, 'max_roll_id' => $max_roll_id, 'driver_lists' => $driver_lists, 'custom_fields' => $custom_fields]);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        $smStudent->parent_id = empty($request->parent_id) ?  $parent->id : $request->parent_id;
                        $smStudent->role_id = 2;
                        ...
                        $smStudent->ifsc_code = $request->ifsc_code;
                        ...
                        if ($smStudent) {
                            $user_info[] = ['email' => $request->email_address, 'name' => $smStudent->full_name, 'id' => $smStudent->id, 'slug' => 'student'];
                        }
                        ...
                        try {
                            ...
                            try {
                                ...
                                foreach ($user_info as $info) {
                                    ...
                                    send_mail($info['email'], $info['name'], 'Login Details', 'backEnd.studentInformation.user_credential');
                                }
                            } catch (Exception $e) {
    ...
    public function studentDetails(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_details', ['classes' => $classes, 'sessions' => $sessions, 'students' => $students]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function studentDetailsSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_details', ['students' => $students, 'sections' => $sections, 'academic' => $academic, 'section_id' => $section_id, 'classes' => $classes, 'class_id' => $class_id, 'name' => $name, 'roll_no' => $roll_no, 'sessions' => $sessions]);
        } catch (Exception $exception) {
    ...
    public function studentView(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_view', ['student_detail' => $student_detail, 'driver_info' => $driver_info, 'fees_assigneds' => $fees_assigneds, 'fees_discounts' => $fees_discounts, 'exams' => $exams, 'documents' => $documents, 'timelines' => $timelines, 'siblings' => $siblings, 'grades' => $grades, 'academic_year' => $academic_year, 'exam_terms' => $exam_terms, 'leave_details' => $leave_details, 'optional_subject_setup' => $optional_subject_setup, 'student_optional_subject' => $student_optional_subject, 'maxgpa' => $maxgpa, 'failgpaname' => $failgpaname, 'custom_field_values' => $custom_field_values, 'academic_history' => $academic_history, 'transections' => $transections]);
        } catch (Exception $exception) {
    ...
    public function studentDelete(Request $request)
    {
        try {
            ...
            try {
                ...
                $isStaff = SmStaff::where('user_id',$parent->user_id)->first();
                ...
                if (count($siblings) == 1) {
                    ...
                    if (empty($isStaff) && !empty($parent_user)) {
                        $parent_user->active_status = 0;
                        $parent_user->save();
                    }
                }

                $class_teacher = SmClassTeacher::whereHas('teacherClass', function ($q) use ($student_detail): void {
                ...
                if ($class_teacher !== null && $class_teacher->teacher) {
    ...
    public function disabledStudent(Request $request)
    {
        try {
            ...
            $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.studentInformation.disabled_student', ['students' => $students, 'classes' => $classes, 'pt' => $pt]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function disabledStudentSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.disabled_student', ['classes' => $classes, 'class_id' => $class_id, 'shift_id' => $shift_id, 'section_id' => $section_id, 'name' => $name, 'admission_no' => $admission_no, 'pt' => $pt, 'students'=>$students]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function previousRecordSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->shift != "") {
                $students->where('previous_shift_id', $request->shift);
            }
            ...
            $year = $request->promote_session;
            $class_id = $request->promote_class;
            $section_id = $request->promote_section;
            $shift_id = $request->shift;

            return view('backEnd.examination.previous_record', ['classes' => $classes, 'exam_types' => $exam_types, 'academic_years' => $academic_years, 'students' => $students, 'year' => $year, 'class' => $class, 'section' => $section, 'shift' => $shift, 'shift_id' => $shift_id, 'class_id' => $class_id, 'section_id' => $section_id, 'year' => $year]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        try {
            return Excel::download(new AllStudentExport, 'all_student_export.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        } catch (Exception $exception) {
    ...
    public function allStudentExportPdf()
    {
        try {
            ...
            return view('backEnd.studentInformation.allStudentExportPdfPrint', ['students' => $students, 'academiYear' => $academiYear]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function unassignedStudent(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.unassigned_student', ['students' => $students, 'classes' => $classes, 'pt' => $pt]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function sortingStudent($class_id)
    {
        try {
            ...
            return view('backEnd.studentInformation.sortingStudent', ['class_id' => $class_id, 'pt' => $pt]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function sortingSectionStudent($class_id, $section_id, $shift_id = null)
    {
        try {
            $class = SmClass::find($class_id);
            $section = SmSection::find($section_id);
            if(shiftEnable()){
                ...
                $pt = trans('student.class').'-'.@$class->class_name.'('.$section->section_name.')'. '('.$shift->name.') ' .trans('student.student_list');
                return view('backEnd.studentInformation.sortingStudent', ['section_id' => $section_id, 'class_id' => $class_id, 'shift_id' => $shift_id, 'pt' => $pt]);
            }else{
                $shift = '';
                $pt = trans('student.class').'-'.@$class->class_name.'('.$section->section_name.')' .trans('student.student_list');
                return view('backEnd.studentInformation.sortingStudent', ['section_id' => $section_id, 'class_id' => $class_id, 'pt' => $pt]);
            }
