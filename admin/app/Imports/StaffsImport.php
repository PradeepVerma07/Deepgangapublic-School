...
class StaffsImport implements ToModel, WithHeadingRow, WithStartRow
{
    ...
    public function model(array $row)
    {
        ...
        if (gv($row, 'date_of_birth')) {
            $dob = $this->parseExcelDate($row['date_of_birth']);
        }

        $date_of_joining = null;
        if (gv($row, 'date_of_joining')) {
            $date_of_joining = $this->parseExcelDate($row['date_of_joining']);
        }
