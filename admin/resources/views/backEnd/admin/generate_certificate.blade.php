...
<script>
    $(document).ready(function() {
        // Handle Print Certificate Button Click
        $('#genearte-certificate-print-button').on('click', function(e) {
            e.preventDefault();
            ...
            if (selectedStudents.length === 0) {
                alert('@lang("admin.please_select_student")');
                return false;
            }
            
            if (!certificateId) {
                alert('@lang("admin.please_select_certificate")');
                return false;
            }
            
            // Build URL with student IDs separated by dash
            var studentIds = selectedStudents.join('-');
            var url = '{{ route("student_certificate_generate", ["s_id" => "STUDENT_IDS", "c_id" => "CERT_ID"]) }}';
            url = url.replace('STUDENT_IDS', studentIds);
            url = url.replace('CERT_ID', certificateId);
        ...
        $('#checkAll').on('change', function() {
            ...
            $('.generate-certificate-print').prop('checked', isChecked);
        });
        
        // Update "Check All" status when individual checkboxes change
        $(document).on('change', '.generate-certificate-print', function() {
            var totalCheckboxes = $('.generate-certificate-print').length;
            var checkedCheckboxes = $('.generate-certificate-print:checked').length;
