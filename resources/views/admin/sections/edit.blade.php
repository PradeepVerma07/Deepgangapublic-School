...
<div class="card">
    <div class="card-header">
        <div class="row">
            <div class="col-lg-6"><h5>Edit Section</h5></div>
            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('menu_id') is-invalid @enderror" id="menu_id" name="menu_id">
                            ...
                            @foreach($menus as $menu)
                                <option value="{{ $menu->id }}" {{ old('menu_id', $section->menu_id) == $menu->id ? 'selected' : '' }}>{{ $menu->title }}</option>
                            @endforeach
                        </select>
                        @error('menu_id')
                ...
                <div class="col-lg-6 mb-3">
                    <div class="form-group">
                        ...
                        <select class="form-control @error('type') is-invalid @enderror" id="type" name="type">
                            ...
                            <option value="editor" {{ old('type', $section->type) == 'editor' ? 'selected' : '' }}>Editor</option>
                            <option value="images_pdf" {{ old('type', $section->type) == 'images_pdf' ? 'selected' : '' }}>Images/PDF</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-lg-12 mb-3" id="editor-content">
                    <div class="form-group">
                        ...
                        <textarea class="form-control editor @error('content') is-invalid @enderror" id="content" name="content">{{ old('content', $section->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div id="file-rows">
                            @if($section->files)
                                @foreach($section->files as $index => $file)
                                    <div class="file-row row mt-2">
                                        <div class="col-lg-5">
                                            <input type="text" class="form-control @error('headings.' . $index) is-invalid @enderror" name="headings[]" placeholder="Heading" value="{{ old('headings.' . $index, $file['heading']) }}">
                                        </div>
                                        <div class="col-lg-4">
                                            <input type="file" class="form-control @error('files.' . $index) is-invalid @enderror" name="files[]" accept="image/*,.pdf">
                                            <a href="{{ asset($file['file_path']) }}" target="_blank" class="me-2 mt-2">
                                                @if(in_array(pathinfo($file['file_path'], PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png']))
                                                    <img src="{{ asset($file['file_path']) }}" alt="Current File" class="section-file">
                                                @else
                                                    View PDF
                                                @endif
                                            </a>
                                            <input type="checkbox" name="delete_files[]" value="{{ $index }}" class="me-2"> Delete
                                        </div>
                                        <div class="col-lg-3">
                            ...
                            @else
                                <div class="file-row row mt-2">
                                    ...
                                    <div class="col-lg-3">
                                        <button type="button" class="btn btn-danger btn-sm remove-row">Remove</button>
                                        <button type="button" class="btn btn-primary btn-sm add-row">Add More</button>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <div class="col-lg-6 mb-3">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control @error('seq') is-invalid @enderror" id="seq" name="seq" value="{{ old('seq', $section->seq) }}" min="0">
                        @error('seq')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
